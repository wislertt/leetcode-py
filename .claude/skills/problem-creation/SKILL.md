...
**GOTCHA — self-referential node classes need `from __future__ import annotations`.** A custom `Node` class in `solution_contents` (N-ary tree, parent-pointer problems) whose own `__init__` annotations reference the class (`children: list[Node] | None = None`) is evaluated at class-body time and fails p-gen lint with F821 `Undefined name 'Node'`. Set `solution_imports: "from __future__ import annotations"` (hit with 428; the remaining unscrapable queue has more parent-pointer/N-ary problems, so this recurs).
