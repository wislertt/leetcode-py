...
- **GOTCHA — replacement pulls misfire when the batch source is `unscrapable`.** Queue entries only leave the waterfall by moving to `UNSCRAPABLE_HANDLED`, so a replacement `--take` called mid-batch re-issues the batch's own top entries instead of new ones (hit 2026-09-01 batch 9: `--take 2` and `--take 6` both returned already-picked 1133-1152). For unscrapable batches, take replacements from the next unclaimed `UNSCRAPABLE_QUEUE` lines directly; `--take` stays correct only for `list`/`new` batches
