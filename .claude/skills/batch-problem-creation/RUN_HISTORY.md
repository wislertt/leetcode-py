...
- 2026-09-01 batch 10: 10 problems (unscrapable queue 1180-1230, full unscrapable web flow, all fetched upfront via curl+GitHub API), parallel 3-concurrent (E1), agent durations 175-400s each (~269s avg, ~45 min total, ~90s/problem wall across 3 lanes), FAILs 0, gates PASS after 2 orchestrator-side fixes, violations 0, notes: 2 queue slugs misnamed again (1215 real = Stepping Numbers not lucky-numbers-in-a-matrix, 1230 real = Toss Strange Coins not count-triplets; existing gotcha caught both, kebab-names fixed at close-out); NEW failure mode: one agent wrote its problem NUMBER instead of dir name into /tmp/batch_name_N.txt, insert_tag.py inserted "1180" into tags.json5 at a bogus bisect position — 2 extra cycles (sed fix + sort fix + pre-commit rerun); suggestions logged in skill-suggestions-log
