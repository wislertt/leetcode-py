...
def run_serialize_deserialize_n_ary(solution_class: type, root_list: list[int | None]):
    root = nary_from_list(root_list)
    codec = solution_class()
    deserialized = codec.decode(codec.encode(root))
    return nary_to_list(deserialized)


def assert_serialize_deserialize_n_ary(result: list[int], expected_list: list[int | None]) -> bool:
